...
- [Pulse Network](https://pulse.theaslangroupllc.com) - 958 pay-per-query intelligence endpoints across 76 verticals (market data, token safety, macro, research) that agents pay via x402. USDC on Solana and Base among 8 accepted networks, no API keys. A single MCP server wraps the whole catalog; machine-readable index at `/.well-known/pulse-catalog.json`.
