...
- [WalletPrint](https://github.com/Loai17/walletprint-sdk) - Behavioral risk scoring SDK for Solana agent wallets. Wraps `@solana/web3.js` `Transaction` and `VersionedTransaction` via `createSolanaWalletPrintMiddleware` — scores proposed transactions against wallet behavioral history before signing and flags new recipients, unusual sizes, and velocity spikes with plain-English reason codes. MIT licensed. `npm install @walletprint/sdk`
