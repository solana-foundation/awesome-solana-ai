...
- [AgentShare](https://agentshare.dev) - DeFi-first Solana intelligence MCP for autonomous agents: Meteora DLMM pool briefs (verdict/risk/momentum), single-pool depth, Solana DEX scout (DefiLlama), plus secondary commerce procurement. Streamable HTTP at `/mcp`; machine-readable discovery at `agent.json` and `/.well-known/mcp/server-card.json`. Free tier API key; x402-ready billing metadata (Cloudflare Monetization Gateway).
