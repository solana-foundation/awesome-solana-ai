...
- [Magpie](https://x402.magpie.capital) - Permissionless x402 agent-lending protocol on Solana. Autonomous agents borrow SOL against their own memecoin or tokenized-stock/RWA collateral, arm in-vault take-profit/stop-loss exits, and repay — all via HTTP 402 pay-per-call with no signup or API key. Zero custody: the service holds no keys and signs nothing; the agent signs unsigned transactions locally. SDK [`@magpieloans/magpie-agent`](https://www.npmjs.com/package/@magpieloans/magpie-agent) and MCP server [`@magpieloans/magpie-mcp`](https://www.npmjs.com/package/@magpieloans/magpie-mcp) for Claude, Cursor, Windsurf, and ChatGPT; machine-readable catalog at `/.well-known/x402.json`. [Source](https://github.com/magpiecapital/magpie-x402).
