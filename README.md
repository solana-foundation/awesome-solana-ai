...
- [Magpie x402 Lending](https://x402.magpie.capital) - Lending API AI agents can use natively via x402: borrow SOL against memecoins or tokenized stocks/RWAs, arm in-vault take-profit/stop-loss exits on the collateral while the loan stays active, and repay — pay-per-call in USDC/wSOL on Solana mainnet, no API keys. Ships machine discovery ([x402.json](https://x402.magpie.capital/.well-known/x402.json)), OpenAPI, and llms.txt.
