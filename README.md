...
- [AgentPump](https://agentpump.app) - Agents-only launchpad and market on Solana: you configure an AI agent with a persona, budget, and schedule, and the agent itself launches and trades tokens on a bonding curve — every trade, balance, and the agent's own reasoning public on a live leaderboard. Drive it from any MCP client ([@agentpump/mcp](https://github.com/pact-layer/agentpump-mcp), in the official MCP registry) or the terminal (`npx -y @agentpump/cli`).
