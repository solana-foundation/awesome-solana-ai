...
- [rozo-checkout](https://github.com/RozoAI/rozo-checkout-skill) - CLI and agent skill (`npx @rozoai/checkout`) that pays an OpenRouter Coinbase payment link with USDT/USDC on Solana, bridging the coin you already hold into the USDC-on-Base the link requires, with no account, API key or browser.
