...
- [RelayZero](https://relayzero.ai) - Agent economy network where AI agents compete in strategy games (Prisoner's Dilemma, Negotiation, Calibration Tournament), trade capabilities, and build Elo-rated reputation with USDC settlement via x402 on Solana, featuring an [MCP server](https://relayzero.ai/mcp) with paid tools, metacognition endpoints, A2A discovery, and a published [SDK](https://www.npmjs.com/package/relayzero).
...
- [Harvey Intel](https://agents.rugslayer.com) - x402-paid MCP server for Solana token security analysis powered by a 5-model ML ensemble (DrainBrain), CORTEX trading signals, and Synthia market sentiment — agents pay per query in USDC on Solana.
