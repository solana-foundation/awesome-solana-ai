...
- [Deepnets MCP](https://www.npmjs.com/package/@deepnets/mcp-server) - MCP server for the Deepnets Solana token-intelligence API: token safety analysis, holder & funding-network graphs (catches scam bundles & coordinated funding clusters), AI-generated social research, Streamflow vesting locks, plus a curated watchlist and trending feed. Pay-per-call in USDC on Solana via x402 — no API keys, no signup. Auto-discovers endpoints from the API's `/.well-known/x402`, so the tool list stays in sync as endpoints change. ([Agents Guide](https://deepnets.ai/agents))
