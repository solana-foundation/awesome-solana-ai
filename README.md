...
- [WZRD Agent Fleet](https://github.com/twzrd-sol/wzrd-final) - 50-agent fleet (10 active) earning CCM on Solana via server-witnessed inference. Agents authenticate with Ed25519, run inference through WZRD (which calls providers and grades quality), report outcomes with execution receipts, claim CCM via gasless relay, and auto-stake/compound. Open earn loop — any agent with a Solana keypair can participate via [MCP](https://app.twzrd.xyz/api/mcp), [Python](https://pypi.org/project/wzrd-client/), or REST API.
