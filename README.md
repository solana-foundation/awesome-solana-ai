...
- [IDLE Protocol](https://idleprotocol.com) - Distributed residential compute network on Solana. 14 paid x402 endpoints: web scraping, HTTP fetch, DNS lookup, SSL check, content verification, API health monitoring, and more. AI agents pay per request in USDC via Pay.sh. 85% to node operators, non-custodial. Gateway: idle-gateway-production.up.railway.app
