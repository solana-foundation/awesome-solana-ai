...
- [wzrd-client](https://github.com/twzrd-sol/wzrd-final) - AI model velocity oracle on Solana. Tracks adoption momentum of 100+ models across HuggingFace, GitHub, OpenRouter, and ArtificialAnalysis via 9 Switchboard feeds on mainnet. `pip install wzrd-client` for model selection; agents earn by reporting routing decisions. ([PyPI](https://pypi.org/project/wzrd-client/), [API](https://api.twzrd.xyz/v1/signals/momentum))
...
- [WZRD MCP Server](https://github.com/twzrd-sol/wzrd-final) - MCP server with 26 tools for real-time AI model velocity signals on Solana. Streamable HTTP at `app.twzrd.xyz/api/mcp`. Covers model selection, momentum queries, and agent-to-protocol interaction via Ed25519 auth.
