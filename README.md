...
- [TWZRD Agent Intelligence](https://intel.twzrd.xyz) - Trust + receipt layer for x402 agents on Solana. A free preflight ReadinessCard scores any counterparty on a wash/Sybil-resistant cross-facilitator reputation corpus (decide before paying); the paid trust endpoint ($0.05 USDC, mainnet) returns the score plus a portable Ed25519-signed V6 receipt verifiable offline. Indexed on x402scan with an MCP server on the official MCP Registry (`pip install twzrd-agent-intel`).
