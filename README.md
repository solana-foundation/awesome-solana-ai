...
- [2s.io](https://2s.io) - Pay-per-call HTTP API for AI agents — an ever-expanding catalog spanning patents, SEC filings, court cases, scientific papers, OFAC sanctions, weather, geocoding, AI utilities, and more. Settled in USDC on Solana or Base via x402, no API keys or signup. Open-ended experiment in maximally-comprehensive agent infrastructure. Live [directory](https://2s.io/api/directory).
