...
- [2s.io](https://2s.io) - Pay-per-call HTTP API for AI agents with dozens of endpoints across patents, court cases, scientific papers, OFAC sanctions, weather, geocoding, ZIP code utilities, and more — settled in USDC on Solana or Base via x402, no API keys or signup. Live [directory](https://2s.io/api/directory).
